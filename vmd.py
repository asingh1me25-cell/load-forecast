...
if datetime_col:
    ...
    print("⚠️ No datetime-like column found! Skipping timestamp creation.")
...
pivot_data = df.pivot_table(values='Load', index='hour', columns='day_of_week', aggfunc='mean')
day_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
# Filter only existing days in the DataFrame
existing_days = [day for day in day_order if day in pivot_data.columns]
if existing_days:
    pivot_data = pivot_data[existing_days]
else:
    print("⚠️ No matching weekday columns found in pivot_data. Using default order.")
...
# ACF
autocorrelation_plot(df['Load'], ax=axes[0, 0], color='blue')
axes[0, 0].set_title('Autocorrelation Function (ACF) - 168 Lags', fontsize=12, fontweight='bold')
axes[0, 0].set_xlabel('Lag (hours)')
axes[0, 0].grid(True, alpha=0.3)
...
print(f"Test R²: {lstm_test_r2:.6f}")
...
print(f"  • After feature engineering: {len(df_model):,}")

print("\n🔄 VMD DECOMPOSITION:")
print(f"  • Number of IMFs: {K}")
print(f"  • Alpha (bandwidth): {alpha}")
...
print(f"  • Model: VMD-XGBoost-LSTM Hybrid")
print(f"  • MAPE: {ensemble_mape_val:.4f}%")
print(f"  • R²: {ensemble_r2:.6f}")
print(f"  • MAE: {ensemble_mae:.2f} MW")
print(f"  • RMSE: {ensemble_rmse:.2f} MW")

print(f"\n  • XGBoost weight: {optimal_w_xgb:.4f}")
print(f"  • LSTM weight: {optimal_w_lstm:.4f}")

print("\n✅ FILES SAVED:")
print("  • 07_vmd_decomposition.png")
print("  • 08_lstm_training_history.png")
...
print(f"Test R²: {xgb_test_r2_v2:.6f}")
...
print(f"Test R²: {lstm_test_r2_v2:.6f}")
...
comparison = pd.DataFrame({
    ...
    'R²': [
        ...
        0.96,
        0.9875,
        results_df_v2[results_df_v2['Model']=='XGBoost V2 (K=6)']['R²'].values[0],
        results_df_v2[results_df_v2['Model']=='LSTM V2 (3-layer)']['R²'].values[0],
        results_df_v2[results_df_v2['Model']=='VMD-V2-Hybrid']['R²'].values[0]
    ]
})
...
improvement_r2 = ((comparison[comparison['Model']=='VMD-V2-Hybrid']['R²'].values[0] - 0.9875) / 0.9875) * 100

print(f"\n" + "="*80)
print("IMPROVEMENT METRICS:")
print("="*80)
print(f"MAPE Improvement: {improvement_mape:.2f}% reduction")
print(f"R² Improvement: {improvement_r2:.2f}% increase")
...
# R² Comparison
r2_vals_compare = comparison['R²'].values
axes[0, 1].bar(range(len(models_compare)), r2_vals_compare, color=colors_compare, edgecolor='black', alpha=0.7, width=0.6)
axes[0, 1].set_title('R² Comparison: V1 vs V2 (Higher is Better)', fontsize=12, fontweight='bold')
...
v2_r2 = results_df_v2[results_df_v2['Model']=='VMD-V2-Hybrid']['R²'].values[0]

print(f"\n📊 V1 (Original) Performance:")
print(f"  • MAPE: {v1_mape:.4f}%")
print(f"  • R²: {v1_r2:.6f}")

print(f"\n📊 V2 (Improved) Performance:")
print(f"  • MAPE: {v2_mape:.4f}%")
print(f"  • R²: {v2_r2:.6f}")
...
if v2_mape < v1_mape:
    print(f"    ({v1_mape:.4f}% → {v2_mape:.4f}%) ✅")
else:
    print(f"    ({v1_mape:.4f}% → {v2_mape:.4f}%) ⚠")

print(f"  ✓ R² improvement: {r2_improvement:.4f}%")
if v2_r2 > v1_r2:
    print(f"    ({v1_r2:.6f} → {v2_r2:.6f}) ✅")
else:
    print(f"    ({v1_r2:.6f} → {v2_r2:.6f}) ⚠")

print(f"\n🔧 CHANGES MADE:")
print(f"  1. VMD: K increased from 5 → 6 IMFs")
print(f"  2. Features: Added 4 new seasonal/holiday features")
print(f"  3. XGBoost: Increased n_estimators (200 → 250)")
print(f"  4. XGBoost: Increased max_depth (6 → 7)")
print(f"  5. LSTM: 2-layer (100-50) → 3-layer (128-64-32)")
print(f"  6. LSTM: Increased epochs (100 → 150)")
print(f"  7. LSTM: Reduced batch size (32 → 24)")
print(f"  8. LSTM: Adjusted learning rate (0.001 → 0.0008)")

print(f"\n💾 SAVED FILES:")
print(f"  • 11_vmd_improved_k6.png")
print(f"  • 12_lstm_v2_training_history.png")
print(f"  • 13_v1_vs_v2_comparison.png")
print(f"  • xgb_model_v2_k6.json")
print(f"  • lstm_model_v2_improved.h5")
print(f"  • model_results_v2.csv")
print(f"  • v1_vs_v2_comparison.csv")
...
print(f"Test R²: {xgb_test_r2_opt:.6f}")
...
print(f"Test R²: {lstm_test_r2_opt:.6f}")
...
for w1, w2 in test_weights:
    ...
    print(f"  W: ({w1:.2f}, {w2:.2f}) → MAPE: {m:.4f}%, R²: {r:.6f}")
...
print("\n" + "="*80)
print("✅ FINAL REAL-WORLD METRICS (Inverse Transformed)")
print("="*80)
...
print("\n📊 IMPROVED XG-LSTM RESULTS (SCALED)")
...
print("\n📊 FINAL IMPROVED XG-LSTM RESULTS (SCALED):")
...
print("\n📊 FINAL IMPROVED XG-LSTM RESULTS (REAL-WORLD):")
